...
const UpcomingBookingCard: React.FC<{ booking: Booking }> = ({ booking }) => {
  ...
  const fetchCoachInfo = async (coachId: number) => {
    try {
      ...
      if (response.ok) {
        ...
        console.error("Failed to fetch coach information");
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (booking.slot.coachId) {
      fetchCoachInfo(booking.slot.coachId);
    }
  }, [booking.slot.coachId]);
        ...
        <HoverCard>
          ...
          <HoverCardContent className="p-4 bg-white rounded-md shadow-md border max-w-xs">
            <div className="flex flex-col space-y-2">
              <span className="font-semibold text-lg">{coach?.name}</span>
